...
int main() {
  ...
  if (child > 0) {
    ...
    sigqueue(getppid(),SIGTERM,s_value_int); //just the same as kill function,
    but we can send additional data too
    */
    struct student adam = {"Barath Adam",
                           2014}; // define a data structure to send
    sleep(1);
    union sigval s_value_ptr;
    s_value_ptr.sival_int = 2019; // the struct data must define in commmon code
